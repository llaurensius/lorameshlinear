...
void loop() {
    ...
    if (nodeId == 1) {
        ...
        // Delay for a random time to allow others to send before attempting to send
        delay(random(100, 1000));  // Random delay between 100ms and 1000ms

        // Sending message to N2 with retry
    ...
    delay(2000); // Delay before the next transmission
}
